...
class TestConfig:
    ...
    def test_config_002(self):
        ...
        """
        config = ProjectConfigReader(Path(__file__).parent / 'projects' / 'jagersfontein.yml')
        ...
        config.errors = config.validate(dict(config)) # re-validate config after modification
