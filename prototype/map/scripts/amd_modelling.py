...
def train_model(clean_feat):
    X = clean_feat[[ "lag1", "lag2", "diff1", "diff2", "local_std" ]] # "lag3",  "month" "rolling_mean"
...
def predict(model, feat):
    X = feat[[ "lag1", "lag2", "diff1", "diff2", "local_std" ]] # , "rolling_mean", "month"
...
def plot_lags_vs_target(clean_feat, amd_feat):
    ...
    ax.plot(
        clean_feat.index,
        clean_feat["AMD_smooth"],
        color="black",
        linewidth=2,
        label="Target (AMD_smooth)"
    )
    ...
    ax.set_title("Clean Water — Target vs Lag Features")
    ax.set_ylabel("AMD index")
    ax.legend(loc="upper right")
    ax.grid(True)
    ...
    ax.set_title("AMD Water — Target vs Lag Features")
    ax.set_ylabel("AMD index")
    ax.set_xlabel("Date")
    ax.legend(loc="upper right")
    ax.grid(True)
...
def main(dir_path, amd_fn, clean_fn):
    ...
    plot_lags_vs_target(clean_feat, amd_feat)
