...
COMPARE_SPEC = importlib.util.spec_from_file_location("compare_insar_insitu", COMPARE_SCRIPT)
...
def test_extracts_all_geopackage_points_with_window_mean(tmp_path):
    ...
    with rasterio.open(
        true_los / "TRUE_LOS_20180101.tif", "w", driver="GTiff",
...
def test_loads_every_point_using_configured_label_column(tmp_path):
    ...
    assert MODULE.load_observation_points(
        points_file, "EPSG:32633", "lable"
    ) == {
