...
def sample_neighbourhood(
    src: rasterio.io.DatasetReader, x: float, y: float, size: int
) -> float:
...
def extract(
    ...
    if project_dir is None:
        ...
        if not root:
            raise ValueError(
                'project_dir is required when --project-dir is not supplied'
            )
    ...
    for tif in files:
        ...
        with rasterio.open(tif) as src:
            if src.crs != raster_crs:
                raise ValueError(
                    f'Raster CRS differs from the first acquisition: {tif}'
                )
            for name, (x, y) in points.items():
