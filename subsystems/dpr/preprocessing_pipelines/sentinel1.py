...
class Sentinel1Pipeline(PreprocessingBasePipeline):
    ...
    def _compute_displacement(self, target_m=10.0):
        ...
        t_dim = next((d for d in disp_ra.dims if d in ("date", "time", "epoch", "pair")), None)
        if t_dim:
            disp_ra = disp_ra.where(disp_ra[t_dim] != disp_ra[t_dim].values[0], other=np.nan)

        # 3. Geocoding & Coordinate Transformation
        self.sbas.compute_geocode(float(target_m))
        self.disp_ll = self.sbas.cropna(self.sbas.ra2ll(disp_ra)).compute() # can be switched to .persist()
    ...
    def _compute_risk(self):
        ...
        if self.disp_ll is None:
            raise RuntimeError("Missing displacement data. Run _compute_displacement first.")
        ...
        try:
            ...
            d_x = "lon" if "lon" in dem.dims else ("longitude" if "longitude" in dem.dims else "x")
            d_y = "lat" if "lat" in dem.dims else ("latitude" if "latitude" in dem.dims else "y")
            dem = dem.rename({d_x: "x", d_y: "y"}).rio.set_spatial_dims(x_dim="x", y_dim="y")
            ...
            dx = abs(np.nanmedian(np.diff(x_coords))) * 111320.0 * np.cos(np.deg2rad(lat0))
            ...
            sample = disp.isel({t_dim: 0}, drop=True)
            slope_da = xr.DataArray(np.zeros(sample.shape), coords=sample.coords, dims=sample.dims)
        ...
        self.failure_flag = xr.where(self.risk_map >= 6, 1, 0).rename("failure_flag").persist()
