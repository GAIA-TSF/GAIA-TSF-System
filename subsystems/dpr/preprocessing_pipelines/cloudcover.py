...
class Sentinel2CloudCoverPipeline(BasePipeline):
    ...
    def __init__(self, metadata_path: str, scl_band: int = 13):
        """
        Initialize the pipeline with the path to a JSON metadata file.
        :param metadata_path: Path to the JSON file (must contain a 'path' key to the Sentinel-2 raster).
        ...
        self.raster_path = self.metadata.get("path")

        if not self.raster_path:
            raise KeyError("The JSON metadata must contain a 'path' key.")
    ...
    def _process_slc_statistics(self):
        ...
        try:
            ...
            # get cover percentages (float 0.0 - 1.0) for classes appearing in the SCL band
