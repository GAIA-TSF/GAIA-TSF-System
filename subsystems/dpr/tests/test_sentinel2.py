...
class TestSentinel2Workflow:
    def manual_test_sentinel2_safe_processor(self):
        ...
        pipeline.run(safe_path, output_folder, roi=roi, target_res=res, resampling_alg=r_alg)

        # Check if the files were created
        filename = os.path.basename(safe_path).replace('.SAFE', '').replace('.zip', '.tiff')
        ...
        assert ds.RasterCount == 13, 'The Geotiff does not contain the right amount of bands.'
        gt = ds.GetGeoTransform()
        output_res = (numpy.abs(gt[1]), numpy.abs(gt[5]))
        assert output_res == res, f'The spatial resolution of the Geotiff {output_res} ' \
        ...
        for key in essential_keys:
            assert key in metadata, (
                f'Key {key} is missing from metadata file.'
            )
