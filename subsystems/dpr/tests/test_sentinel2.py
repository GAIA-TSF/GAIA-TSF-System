...
class TestSentinel2Workflow:
    ...
    def test_cloudcover(self):
        ...
        assert 'cloud_cover_pct' in processed_metadata, "Key 'cloud_cover_pct' missing from metadata."

        # Check if the value is a valid number between 0 and 1
        cloud_pct = processed_metadata['cloud_cover_pct']
        assert isinstance(cloud_pct, (int, float)), f"Expected number for cloud cover, got {type(cloud_pct)}"
        assert 0 <= cloud_pct <= 1, f"Cloud cover percentage {cloud_pct} is out of bounds (0-1)."
