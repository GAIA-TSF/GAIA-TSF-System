...
from subsystems.map.utils.artifacts import regression_metrics, write_diagnostics, write_json
...
class LearningPipeline:
    ...
    def run(self) -> dict[str, Any]:
        ...
        selector = StablePixelSelector(float(self.config["baseline_model"]["stable_pixel_std_threshold"]))
        stable_mask = selector.select(loaded.features[target_feature], loaded.mask)
        builder = DatasetBuilder()
        stable_dataset = builder.build(loaded, feature_names, target_feature, stable_mask)
        ...
        datasets = builder.split_temporal(
            stable_dataset, float(split["train_ratio"]), float(split["validation_ratio"]), float(split["test_ratio"]),
        )
        self._seed()
        ...
        metrics = {
            "training": regression_metrics(datasets.train.targets, model.predict(datasets.train.features).y_pred),
        ...
        write_diagnostics(models_dir, datasets.validation.targets, validation, datasets.validation.dates, datasets.validation.time_indices)
        metadata = {
            "experiment": self.config.get("experiment", {}), "variable": variable_name,
            "model": model_name, "feature_names": feature_names, "target_feature": target_feature,
            "stable_pixel_count": int(np.count_nonzero(stable_mask)), "sample_counts": {
                "train": int(datasets.train.targets.size), "validation": int(datasets.validation.targets.size), "test": int(datasets.test.targets.size),
    ...
    def _seed(self) -> None:
        seed = int(self.config.get("training", {}).get("random_seed", self.config.get("experiment", {}).get("seed", 42)))
        random.seed(seed)
        np.random.seed(seed)
