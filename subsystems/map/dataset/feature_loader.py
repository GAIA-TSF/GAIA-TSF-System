...
class FeatureLoader:
    ...
    def load(self, feature_names: list[str]) -> LoadedFeatures:
        ...
        if rasterio is None:
            raise RuntimeError("FeatureLoader requires the Rasterio dependency for GeoTIFF input.")
        ...
        for name in feature_names:
            ...
            with rasterio.open(path) as source:
                ...
                current_grid = RasterGrid(
                    source.crs, source.transform, source.height, source.width, source.nodata,
                )
            ...
            if values.shape[0] > 1:
                if temporal_dates is not None and len(temporal_dates) != values.shape[0]:
                    raise ValueError(f"Feature {name} has incompatible time bands: {path}")
        ...
        arrays = {name: self._broadcast(values, stack_count, name) for name, values in arrays.items()}
        with rasterio.open(self.mask_path) as source:
            mask = source.read(1).astype(bool)
            if mask.shape != (grid.height, grid.width):
                raise ValueError("TSF mask dimensions do not match engineered features.")
    ...
    def _dates_for(self, path: Path, descriptions: tuple[str | None, ...]) -> tuple[str, ...]:
        ...
        return tuple(value or f"acquisition_{index:04d}" for index, value in enumerate(descriptions))
    ...
    @staticmethod
    def _validate_grid(reference: RasterGrid, candidate: RasterGrid, path: Path) -> None:
        if (reference.height, reference.width, reference.transform, reference.crs) != (
            candidate.height, candidate.width, candidate.transform, candidate.crs,
        ):
