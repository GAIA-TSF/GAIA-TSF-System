...
def write_diagnostics(
    ...
    limits = [float(np.nanmin((observed, predicted))), float(np.nanmax((observed, predicted)))]
    axis.plot(limits, limits, "k--", linewidth=1)
    axis.set(xlabel="Observed", ylabel="Predicted", title="Observed vs predicted")
    figure.savefig(output_dir / "observed_vs_predicted.png", dpi=150, bbox_inches="tight")
    plt.close(figure)
    ...
    observed_mean = [float(np.mean(observed[time_indices == index])) for index in unique_times]
    predicted_mean = [float(np.mean(predicted[time_indices == index])) for index in unique_times]
    ...
    figure.savefig(output_dir / "timeseries_comparison.png", dpi=150, bbox_inches="tight")
    plt.close(figure)

    figure, axis = plt.subplots()
    axis.plot(labels, [float(np.mean(residuals[time_indices == index])) for index in unique_times])
    axis.tick_params(axis="x", rotation=45)
