...
class ResidualAnalyzer:
    ...
    def analyze(self, dataset: Dataset, predictions: np.ndarray) -> ResidualResult:
        ...
        statistics: dict[str, float | int] = {
            ...
            "max": float(np.max(finite)), "mae": float(np.mean(np.abs(finite))),
            "rmse": float(np.sqrt(np.mean(np.square(finite)))),
        }
        return ResidualResult(residuals, stack, statistics)

    def write(self, result: ResidualResult, dataset: Dataset, output_dir: Path) -> list[Path]:
        ...
        (output_dir / "residual_statistics.json").write_text(
            json.dumps(result.statistics, indent=2, sort_keys=True), encoding="utf-8",
        )
        return paths
    ...
    def restore_stack(dataset: Dataset, values: np.ndarray) -> np.ndarray:
        ...
        stack = np.full((len(dataset.dates), dataset.grid.height, dataset.grid.width), np.nan)
    ...
    @staticmethod
    def _write_raster(path: Path, values: np.ndarray, dataset: Dataset, description: str) -> None:
        if rasterio is None:
            raise RuntimeError("Residual product writing requires the Rasterio dependency.")
        ...
        with rasterio.open(
            path, "w", driver="GTiff", height=dataset.grid.height, width=dataset.grid.width,
            count=1, dtype="float32", crs=dataset.grid.crs, transform=dataset.grid.transform,
            nodata=nodata,
        ) as raster:
